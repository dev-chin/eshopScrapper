...
Path("scrap/US").mkdir(parents=True, exist_ok=True)

# Load the JSON file with NSU IDs
with open("ValidNsuIds/US.json", "r", encoding="utf-8") as f:
    nsu_ids = json.load(f)
...
for nsu_id in nsu_ids:
    if (nsu_id in titledb_IDs): 
        continue
    if (os.path.isfile("scrap/US/%d.json" % nsu_id) == True):
        continue
    # Create the URL
    url = f"https://ec.nintendo.com/US/en/titles/{nsu_id}"
    ...
    try:
        ...
        if match:
            ...
            # Save to file
            output_file = f"scrap/US/{nsu_id}.json"
